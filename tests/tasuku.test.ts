...
test("task - return number", async () => {
    const { result } = await queue("Some task", async () => 1 + 1);
    expect(result).toBe(2);
});

// test("task - return string", async () => {
//     const { result } = await queue("Some task", async () => "some string");
...
//     const someTask = await queue("Some task", async ({ task }) => {
...
//     const groupTasks = await task.group((task) => [task("number", async () => 123), task("string", async () => "hello"), task("boolean", async () => false)]);

//     expect(groupTasks.results).toEqual([123, "hello", false]);
// });
